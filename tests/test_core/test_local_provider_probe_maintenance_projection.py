...
def _request(**overrides: object) -> dict[str, object]:
    request: dict[str, object] = {
        "request_id": "local-provider-probe-maintenance-projection:aegis:1",
        ...
        "source_refs": [{"ref_id": "manual-smoke:lm-studio:models:1", "ref_type": "probe_projection"}],
        ...
        "local_provider_probe_projection_ref": {
            "ref_id": "local-provider-probe-projection:1",
            "probe_result_class": "unreachable_negative_candidate",
            "projection_status": "negative_candidate_projected",
        },
        "probe_result_class": "unreachable_negative_candidate",
        "projection_status": "negative_candidate_projected",
        "display_severity_class": "warning",
        "limitations": ["projection only"],
        "unknowns": ["provider reachability may change"],
        "authority": False,
        "runtime_dispatch_allowed": False,
        "execution_permission": LOCAL_PROVIDER_PROBE_MAINTENANCE_PROJECTION_EXECUTION_PERMISSION,
    }
...
def _assert_non_authority(decision: object) -> None:
    ...
    assert decision.execution_permission == LOCAL_PROVIDER_PROBE_MAINTENANCE_PROJECTION_EXECUTION_PERMISSION
    assert decision.approval_grant is False
    assert decision.capability_grant is False
    assert decision.lease_grant is False
    assert decision.evidence_provided_by_maintenance_projection is False
...
def test_valid_unreachable_manual_smoke_projection_becomes_negative_candidate_only() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(_request())

    assert decision.contract_version == LOCAL_PROVIDER_PROBE_MAINTENANCE_PROJECTION_VERSION
    assert decision.projection_status == "negative_candidate_requires_operator_retry_review"
    assert decision.api_projection_semantics == (
        "unreachable_negative_candidate_display_only_not_runtime_failure_or_health_mutation"
    )
    assert decision.display_status_candidate == "retry_requires_operator_approval_notice"
...
def test_valid_timeout_projection_becomes_timeout_notice_only() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
            local_provider_probe_projection_ref={"probe_result_class": "timeout_negative_candidate"},
    ...
    assert decision.projection_status == "negative_candidate_requires_operator_retry_review"
    assert decision.api_projection_semantics == (
        "timeout_negative_candidate_display_only_not_runtime_failure_or_health_mutation"
    )
    assert decision.runtime_health_mutated is False
    assert decision.retry_authorized is False


def test_valid_model_list_success_projection_is_metadata_candidate_only() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
            projection_api_source_class="local_provider_probe_projection",
            api_exposure_readiness_class="api_projection_metadata_only",
            maintenance_category_class="local_model_list_metadata",
            display_contract_class="not_model_availability_proof_notice",
            local_provider_probe_projection_ref={"probe_result_class": "model_list_success_candidate"},
            probe_result_class="model_list_success_candidate",
            display_severity_class="info",
        )
    )

    assert decision.projection_status == "model_list_metadata_projection_ready"
    assert decision.api_projection_semantics == (
        "model_list_metadata_display_only_not_model_availability_proof"
    )
    assert decision.model_availability_verified is False
    _assert_non_authority(decision)


def test_valid_empty_model_list_projection_is_empty_candidate_only() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
            api_exposure_readiness_class="api_projection_metadata_only",
            maintenance_category_class="local_model_list_metadata",
            display_contract_class="not_runtime_failure_notice",
            local_provider_probe_projection_ref={"probe_result_class": "empty_model_list_candidate"},
            probe_result_class="empty_model_list_candidate",
            display_severity_class="low",
        )
    )

    assert decision.projection_status == "empty_model_list_projection_ready"
    assert decision.api_projection_semantics == (
        "empty_model_list_display_only_not_runtime_failure_or_availability_proof"
    )
    assert decision.runtime_health_mutated is False
    assert decision.model_availability_verified is False


def test_retry_guidance_does_not_authorize_retry() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(_request())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("projection_api_source_class", "missing_projection_api_source_class"),
        ("api_exposure_readiness_class", "missing_api_exposure_readiness_class"),
        ("maintenance_category_class", "missing_maintenance_category_class"),
        ("consumer_surface_class", "missing_consumer_surface_class"),
        ("display_contract_class", "missing_display_contract_class"),
        ("namespace", "missing_namespace"),
    ],
)
def test_missing_required_fields_block(field: str, reason: str) -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(_request(**{field: None}))
...
def test_missing_probe_projection_reference_blocks() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
            local_provider_probe_projection_ref=None,
            probe_result_class=None,
            projection_status=None,
        )
    )

    _assert_blocked(decision, "missing_probe_projection_reference")


@pytest.mark.parametrize(
    ("field", "value", "reason"),
    [
        ("projection_api_source_class", "unsupported", "unsupported_projection_api_source_class"),
        ("api_exposure_readiness_class", "unsupported", "unsupported_api_exposure_readiness_class"),
        ("maintenance_category_class", "unsupported", "unsupported_maintenance_category_class"),
        ("consumer_surface_class", "unsupported", "unsupported_consumer_surface_class"),
        ("display_contract_class", "unsupported", "unsupported_display_contract_class"),
...
def test_unsupported_taxonomy_values_block(field: str, value: str, reason: str) -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(_request(**{field: value}))
...
def test_unknown_taxonomy_values_block() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
            projection_api_source_class="unknown",
            api_exposure_readiness_class="unknown",
            maintenance_category_class="unknown",
            consumer_surface_class="unknown",
            display_contract_class="unknown",
            local_provider_probe_projection_ref={"probe_result_class": "unknown"},
            probe_result_class="unknown",
        )
    )

    assert "unknown_projection_api_source_blocked" in decision.failure_reasons
    assert "unknown_api_exposure_readiness_blocked" in decision.failure_reasons
    assert "unknown_maintenance_category_blocked" in decision.failure_reasons
    assert "unknown_consumer_surface_blocked" in decision.failure_reasons
    assert "unknown_display_contract_blocked" in decision.failure_reasons
...
def test_truthfulness_overclaims_are_rejected(field: str, reason: str) -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(_request(**{field: True}))

    _assert_blocked(decision, reason)


def test_unreachable_projection_is_not_runtime_failure_or_health_mutation() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(_request())

    assert "runtime_failure" not in decision.truthfulness_classification
    assert decision.maintenance_health_mutated is False
    assert decision.runtime_health_mutated is False
    assert decision.provider_health_verified is False


def test_display_contract_mismatch_blocks_overclaiming_negative_as_metadata() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(display_contract_class="metadata_candidate_notice")
    )

    _assert_blocked(decision, "negative_candidate_display_contract_required")


def test_model_list_must_be_labeled_as_not_model_availability_proof() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
            api_exposure_readiness_class="api_projection_metadata_only",
            maintenance_category_class="local_model_list_metadata",
            display_contract_class="metadata_candidate_notice",
            local_provider_probe_projection_ref={"probe_result_class": "model_list_success_candidate"},
            probe_result_class="model_list_success_candidate",
        )
    )

    _assert_blocked(decision, "model_list_availability_notice_required")


def test_auto_mode_cannot_select_model_from_maintenance_projection() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(auto_mode_selection_performed=True)
    )

    _assert_blocked(decision, "auto_mode_selection_denied")


def test_frontend_and_api_consumers_cannot_become_authority() -> None:
    frontend_decision = validate_local_provider_probe_maintenance_projection_request(
        _request(frontend_authority=True)
    )
    api_decision = validate_local_provider_probe_maintenance_projection_request(
        _request(api_authority=True)
    )

    _assert_blocked(frontend_decision, "frontend_authority_not_allowed")
    _assert_blocked(api_decision, "api_authority_not_allowed")


def test_safe_probe_projection_decision_can_be_referenced() -> None:
    projection_decision = validate_local_provider_probe_projection_request(
        {
    ...
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(local_provider_probe_projection_ref=None, probe_result_class=None),
        local_provider_probe_projection_decision=projection_decision,
    )

    assert decision.projection_status == "negative_candidate_requires_operator_retry_review"
    assert [ref.label for ref in decision.related_references] == ["local_provider_probe_projection"]
    assert all(ref.reference_only for ref in decision.related_references)
    assert projection_decision.runtime_health_mutated is False


@pytest.mark.parametrize(
    ("related_name", "kwargs"),
    [
        ("local_provider_probe_projection_decision", {"provider_health_verified": True}),
        ("local_provider_probe_runner_decision", {"live_probe_performed": True}),
        ("local_provider_probe_live_gate_decision", {"real_endpoint_probed": True}),
        ("local_provider_probe_mock_runner_decision", {"model_availability_verified": True}),
        ("local_provider_probe_wiring_decision", {"api_route_added": True}),
        ...
        ("audit_query_layer_decision", {"runtime_command_added": True}),
...
def test_execution_authority_health_mutation_and_proof_flags_are_rejected(
    field: str,
    reason: str,
) -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
...
def test_future_gated_projection_remains_non_authoritative() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
            projection_api_source_class="maintenance_scan_future_projection",
            api_exposure_readiness_class="future_gated",
            maintenance_category_class="provider_probe_status",
            display_contract_class="non_authoritative_status_card",
            local_provider_probe_projection_ref={"probe_result_class": "not_executed"},
            probe_result_class="not_executed",
        )
    )

    assert decision.projection_status == "future_gated_projection_ready"
    assert decision.api_route_added is False
    assert decision.runtime_command_added is False
    _assert_non_authority(decision)
...
def test_input_and_related_decisions_are_not_mutated() -> None:
    request = _request(
        source_refs=[{"ref_id": "source", "nested": {"value": 1}}],
        local_provider_probe_projection_ref={
            "probe_result_class": "unreachable_negative_candidate",
            "nested": {"value": 1},
        },
...
def test_non_mapping_request_blocks_without_side_effects() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(None)

    _assert_blocked(decision, "missing_request")
    assert decision.projection_input is None


def test_output_never_sets_runtime_model_health_or_api_flags_even_when_blocked() -> None:
    decision = validate_local_provider_probe_maintenance_projection_request(
        _request(
