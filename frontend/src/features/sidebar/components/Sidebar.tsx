...
export const Sidebar = () => {
  ...
  return (
    <aside className="w-16 lg:w-60 flex flex-col border-r border-white/10 bg-background/95 z-50">
...
const NavItem = ({ icon, label, active, onClick }: any) => (
  <button
    ...
    aria-label={label}
    onClick={onClick}
    className={`w-full flex items-center gap-3 p-2.5 rounded-md cursor-pointer transition-colors duration-150 group relative overflow-hidden ${active ? 'bg-accent/10 text-accent border border-accent/20' : 'text-foreground/45 hover:bg-white/[0.04] hover:text-white border border-transparent'}`}
  >
