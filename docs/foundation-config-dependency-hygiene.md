# Foundation Config / Dependency Hygiene Hardening
## Decision
...
This sprint hardens older configuration and dependency surfaces so they do not
...
Environment variables or a local `.env` can still provide older model names or
...
`src/aegis/models/llm.py` remains a older provider client. It is now fail-closed
