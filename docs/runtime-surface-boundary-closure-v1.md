# Runtime Surface Boundary Closure v1
...
- Generic `click` remained present in policy dispatch names, tool registry/specs, config, executor browser handling, and the legacy `ClickTool`.
...
- Left `ClickTool` only as a quarantined legacy stub that returns an error and performs no browser click.
...
- Legacy parser surfaces can still produce `click` intents for guard/non-executable handling; this is acceptable only because runtime dispatch is now quarantined.
...
Recommended next sprint: `Runtime State Timeout / Safe Fallback Contract v1`.
