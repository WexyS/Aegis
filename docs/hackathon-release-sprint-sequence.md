# Hackathon release Sprint Sequence

## Decision

Decision: `HACKATHON_RELEASE_SPRINT_SEQUENCE_DOCUMENTED_ONLY`
...
### D0 - Docs / Agent Governance Realignment
Align agent instructions and planning docs so Hackathon release work can proceed
without weakening Aegis invariants.

Gate: `AGENTS.md`, `hackathon-release-scope.md`, and `AEGIS_VISION.md` clearly
separate operational instructions, RC scope, and long-term vision.

### S0 - Foundation + RC Readiness Inventory
Inventory current backend, frontend, API, runtime, tests, and docs against the
declared Hackathon release scope.

Gate: identify what already works, what is missing, what is blocked, and what
must be future-gated.

### S1 - Memory OS Core Backend + API
...
Gate: Tier-0 must-work criteria in `docs/hackathon-release-scope.md` pass or claims
are narrowed.
