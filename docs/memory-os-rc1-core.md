...
Decision: MEMORY_OS_RC1_CORE_BACKEND_API

Memory OS RC1-Core is the first real local Memory OS backend for the Aegis
Hackathon RC track. It is intentionally small: explicit API/store requests can
...
`propose` validates RC1-Core rules first, then calls the existing Memory
...
Unknown sensitivity is not accepted for new memories in RC1-Core.
