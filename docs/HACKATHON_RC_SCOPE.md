...
Decision: `HACKATHON_RC_SCOPE_DOCUMENTED_ONLY`
...
Aegis Hackathon RC is a local-first AI Mission Control Workspace that shows a
...
Every feature claimed as part of Hackathon RC must work completely inside its
...
- Full Memory OS beyond RC1-Core
...
## Memory OS RC1-Core Scope

Memory OS RC1-Core is a governed memory proposal loop, not full autonomous
memory.
...
## AutoPilot RC1-Core Scope

AutoPilot RC1-Core is a read-only audit and report producer, not autonomous
execution.
...
## Deterministic Society Session RC1 Scope

Deterministic Society Session RC1 is a deterministic session artifact, not a
