# Older Router Model-Call Boundary Guard
## Decision
...
This sprint hardens the older router/orchestrator/parser boundary so older
...
The older router historically selected a `planner_model` from simple text
...
That older metadata must not bypass the newer post-foundation contracts:
...
`RoutingVerdict.planner_model` may still be populated for older metadata, but
...
The orchestrator no longer forwards the older router model hint into
`IntentParser.parse(...)`.

`IntentParser.parse(..., model=...)` treats `model` as an ignored older hint.
...
- parser ignores older model hints even when `agent_loop` is enabled
...
- orchestrator does not pass older router hints to the parser
- older hints do not create execution, evidence, verifier success, approval,
  lease, or capability grants

## Remaining Risks

The older router still computes a `planner_model` string for compatibility.
