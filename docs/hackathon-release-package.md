# Hackathon release Release Package
Decision: `HACKATHON_RELEASE_PACKAGE_FREEZE`
...
Aegis Hackathon release: local-first Mission Control for governed memory,
...
- Previous sprint decision:
  `S5_RELEASE_GOLDEN_PATH_SMOKE_PASSED_WITH_RUNBOOK`
...
- Mission Control tab: `Hackathon release`
...
`docs/hackathon-release-demo-runbook.md`.
...
- `Hackathon release Mission Control` renders.
...
- "Memory OS Core governs explicit local memory proposals."
- "AutoPilot Core performs a read-only local audit."
...
`docs/hackathon-release-validation-manifest.md` and in the final sprint report.
...
`docs/hackathon-release-demo-runbook.md`, then return to the UI to show rendered
state.
