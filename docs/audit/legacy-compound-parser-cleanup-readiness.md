# Legacy Compound Parser Cleanup Readiness
...
**Scope:** Readiness/audit plus focused parser tests only. No legacy parser removal, no executor/orchestrator/frontend changes, no click implementation, no target resolution, no LLM planner, and no broad cleanup.
...
**Recommended state:** deprecate conceptually, but keep the legacy compound parser in code for now.
...
- `tests/test_intent/test_parser.py`: legacy flag-off behavior and an older compound browser search expectation.
- `tests/test_intent/test_parser_deterministic_decomposition_smoke.py`: default-on, env override, fallback, and legacy-tail protection tests.
...
| --- | --- | --- | --- |
| `google aç ve python ara` | `open_url(google)` + `search_web(query=python)` via normal legacy split rules | safely falls back to legacy parser | Not dependent on `_parse_compound_app_search()` |
| `youtube aç ve müzik ara` | `open_url(youtube)` + `search_web(query=müzik)` via normal legacy split rules | safely falls back to legacy parser | Not dependent on `_parse_compound_app_search()` |
...
| `unknownapp aç ve python ara` | deterministic non-executable `unknown`, `clarification_required` | unsafe legacy behavior not preserved | Must not fall back to legacy executable behavior |
...
| known site names | `google aç ve python ara`, `youtube aç ve müzik ara` | normal split rules -> `open_url(site)` + `search_web(query=...)` | Low for compound cleanup, but still relies on legacy split parser |
...
These tests intentionally make legacy cleanup fail until equivalent deterministic support exists or the behavior is explicitly retired.

---

## Blockers

- Legacy-only compound search tails still exist: `araması yap`, `bul`, `googlela`.
...
After that, a narrow **Legacy Tail Deterministic Coverage** sprint can decide whether to support or retire `araması yap`, `bul`, `googlela`, and English `find` before any removal attempt.
