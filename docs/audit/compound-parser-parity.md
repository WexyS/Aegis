# Compatibility Compound Parser Parity Audit

**Date:** 2026-05-19

**Goal:** Compare older compound parsing with feature-flagged deterministic decomposition before removing, bypassing, or default-enabling any parser path.

**Scope:** Audit/report only. No compatibility parser cleanup, no executor/orchestrator/frontend changes, no click implementation, no target resolution, and no LLM planner work.
...
- older behavior: `ENABLE_DETERMINISTIC_DECOMPOSITION=false`
...
| Input | Older result | Deterministic result | Same / Different | Expected future behavior | Risk if older removed |
| --- | --- | --- | --- | --- | --- |
| `brave açıp python nedir ara` | `open_app(brave)` + `search_web(query=python nedir, browser=brave)` via `compound_app_search` | `open_app(brave)` + `search_web(query=python nedir, browser=brave)` with deterministic metadata | Same behavior, different metadata | Deterministic path should replace older for this case | Low |
| `chrome aç ve python nedir ara` | `open_app(chrome)` + `search_web(query=ve python nedir, browser=chrome)` | `open_app(chrome)` + `search_web(query=python nedir, browser=chrome)` | Different | Deterministic path is safer and cleaner; older preserves connector in query | Low; removal fixes a older query bug |
| `notepad açıp merhaba yaz` | `open_app(notepad)` only | `open_app(notepad)` + `type(text=merhaba, _require_focus=notepad)` | Different | Deterministic path is correct; older drops the type step | Low; removal improves behavior |
...
| `unknownapp aç ve python ara` | `open_app(unknownapp)` + `search_web(query=python)` | non-executable `unknown` with `clarification_required` | Intentionally different | Deterministic behavior is safer; do not preserve unknown-app execution | Medium if older remains enabled for this pattern |
| `brave aç ve ilk sonuca tıkla` | `open_app(brave)` + `unknown` | non-executable `unknown` with `clarification_required` | Intentionally different | Deterministic behavior is safer; no blind click or partial open should execute | Medium if older remains enabled for click-like compounds |
...
| `merhaba` | `general_chat` | `general_chat` | Same | Keep compatibility fallback when decomposition returns `None` | None |

---

## Older Coverage Already Matched

The deterministic path already covers the core safe older app-search cases:
...
For `brave açıp python nedir ara`, deterministic output is behaviorally equivalent to older and adds structured metadata.

---

## Older Behaviors Not Covered Exactly

These differences are intentional and should not block future deprecation:

- Older may omit `_require_focus` for type steps.
- Older may omit `browser` for English search compounds.
- Older may parse incomplete open+type commands as only `open_app`.
- Older may include connector text such as `ve` in a search query.
- Older may execute `open_app(unknownapp)` instead of asking for clarification.
...
Most of these are already protected by deterministic decomposition smoke tests. A cleanup sprint should add any missing parity tests before deleting older code.

---

## Recommendation

**Keep compatibility parser for now. Deprecate later. Cleanup later.**
