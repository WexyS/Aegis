...
Decision: AUTOPILOT_CORE_BACKEND_API

AutoPilot Core is the first real Hackathon release backend path for a safe,
...
AutoPilot Core uses Python filesystem metadata APIs only. It does not:
...
AutoPilot Core does not perform model-based interpretation or deep security
analysis.

## Context Preflight

S2 uses an explicit preflight object:
