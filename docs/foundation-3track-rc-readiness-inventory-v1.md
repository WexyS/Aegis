# Foundation + 3-Track RC Readiness Inventory v1
...
| Replay engine | `src/aegis/replay/replay_engine.py` | implemented_untested_for_RC | Reads replay files and reconstructs traces in tests, but RC backup/restore/replay operational flow is not release-ready by itself. |
...
| Context Policy | `src/aegis/core/context_policy.py` | integration_ready_for_RC_preflight | Tested metadata validator for privacy/provider/budget preflight; no retrieval or model routing. |
...
- Verifier infrastructure exists for runtime actions; report infrastructure for
  AutoPilot RC1 is missing as a real report producer.

## 5. Context Policy / Context Budget Readiness

Classification: `integration_ready_for_RC_preflight`
...
- `docs/context-retrieval-provider-context-budget-v1.md`
- `docs/context-compiler-read-only-contract-implementation-v1.md`
- `docs/context-compiler-read-only-integration-readiness-v1.md`
...
5. AutoPilot RC1-Core can declare and validate:
...
6. Society Session RC1 can consume only approved AutoPilot/Memory/report outputs
...
- `docs/memory-governance-memory-os-contract-v1.md`
- `docs/memory-governance-memory-os-design-v1.md`
...
What S1 must build for Memory OS RC1-Core:
...
Current missing pieces for AutoPilot RC1-Core:
...
Classification: `implemented_tested_for_runtime`, `needs_RC_extensions`
...
- It checks LM Studio `/v1/models`, but Hackathon RC should not require local
...
- `docs/post-foundation-architecture-roadmap-v1.md`: older roadmap/checkpoint
  references foundation-era state and expansion order.
- `docs/memory-governance-memory-os-design-v1.md`: design-only Memory OS
  framing should be cross-referenced from S1, not rewritten in S0.
- `docs/context-compiler-read-only-integration-readiness-v1.md`: context
  compiler remains integration readiness and should be reconciled with RC
  Context Budget preflight language later.
- `docs/mission-control-dry-run-ux-contract-v1.md`: older dry-run UX boundary
  may conflict with S4 premium UI terminology unless scoped carefully.
- `docs/frontend-design-system-playwright-visual-pipeline-readiness-v1.md`:
...
1. S1 - Memory OS RC1-Core Backend + API + minimal UI wiring contract
2. S2a - Repo Source Inventory Implementation for AutoPilot RC1-Core
3. S2b - AutoPilot RC1-Core Report API/UI surface
...
S1 can begin with a narrow Memory OS RC1-Core implementation sprint. S2 should
