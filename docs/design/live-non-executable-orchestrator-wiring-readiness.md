# Live Non-Executable Orchestrator Wiring Readiness
...
- reject `ACTION_STARTED`, `ACTION_COMPLETED`, `ACTION_FAILED`,
  `ACTION_CANCELLED`, and older `APPROVAL_REQUIRED`;
...
- `commands.pending_approvals` from `ApprovalManager` for older and approved
...
## Older APPROVAL_REQUIRED Coexistence

Older `APPROVAL_REQUIRED` remains command-governance shaped.

Current emitters:

- `orchestrator.process(...)` calls `ws_bridge.emit_approval_required(...)`
  during compatibility preflight approval;
...
- do not remove older `APPROVAL_REQUIRED` yet;
- do not overload older `APPROVAL_REQUIRED`;
...
emitting both older `APPROVAL_REQUIRED` and new `APPROVAL_REQUESTED` for the
same guard decision.
...
Default enforcement must also reconcile compatibility preflight approval. A medium-risk
older `click` can still be captured by the old `APPROVAL_REQUIRED` preflight.
...
- no older `APPROVAL_REQUIRED` for new generic click quarantine;
- ready `read_file` and ready `open_app` still reach executor;
- existing approved older command tests either migrate or remain explicitly
...
- older `APPROVAL_REQUIRED` and new `APPROVAL_REQUESTED` separation is
...
- migrate or fence older `APPROVAL_REQUIRED` preflight to avoid duplicate
  approval events;
...
Runtime Non-Executable Live Append Helper:
